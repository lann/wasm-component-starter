...
| [examples/apps/browser-tgz-maker](examples/apps/browser-tgz-maker) | The same `tar-archiver` component transpiled with `jco` and driven from the **browser** over JSPI, gzipping files through an **imported** async streaming `compressor` interface backed by the browser's `CompressionStream`. Demonstrates an async streaming import flowing guest → host → guest, returning a stream from an async export via `wit_bindgen::spawn`, and mapping a component import to a JS adapter with `jco --map`. |
