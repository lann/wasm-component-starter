//! cli-app component: a CLI that reads an HTML file (or fetches a URL), streams
//! it to the `metadata-parser` extractor component, and prints the metadata it
//! gets back.
//!
...
wit_bindgen::generate!({
    // Bindings for the `cli-app` world (`import extractor`). `extract` is
    // `async` in WIT so it is lowered to an `async fn` we can `.await`. The
    // component-type custom section emitted here is merged with the one from
    // `wasip3`'s `command::export!` (and `std`'s own `wasi:*@0.2.0` imports)
    // when the `wasm32-wasip2` linker assembles the component.
    world: "cli-app",
    path: "../wit",
});

use example::async_composition::extractor;
...
impl wasip3::exports::cli::run::Guest for Component {
    async fn run() -> Result<(), ()> {
        let Some(source) = std::env::args().nth(1) else {
            eprintln!("usage: async-composition <file-path-or-url>");
            return Err(());
        };
