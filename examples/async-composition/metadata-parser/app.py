...
implements the `extractor` interface from `../wit/world.wit`: it receives the
document as an incoming byte `stream`, feeds it to Python's built-in
`html.parser`, and emits `(key, value)` metadata pairs on an outgoing `stream`
