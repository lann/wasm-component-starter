...
A CLI (`cli-app`, Rust) reads an HTML file (or fetches a URL) and streams its
bytes to an extractor (`metadata-parser`, Python). The extractor parses the
document with Python's built-in `html.parser` and streams `(key, value)`
metadata pairs back. Neither side buffers the whole document or the whole result
set — data flows through component-model `stream`s in both directions.

```
┌────────────────────┐   stream<u8>            ┌──────────────────────┐
│  cli-app (Rust)    │ ──────────────────────► │  metadata-parser     │
│  wasm32-wasip2     │                         │  (Python)            │
│  + wasip3 run      │ ◄────────────────────── │  componentize-py     │
└────────────────────┘  stream<tuple<…,…>>     └──────────────────────┘
...
## The async pitfall (and why `cli-app` is built the way it is)
...
To export an **async** `run` instead, `cli-app` is built as a `cdylib` (not a
`bin`) and uses the [`wasip3`](https://crates.io/crates/wasip3) crate's
...
When the argument starts with `http://` or `https://`, `cli-app` fetches it
over `wasi:http` instead of reading from disk. The request goes through
`wasip3::http::client::send`, which is `async` — another import an async `run`
can drive but a synchronous `main` cannot.
...
| Path | Role |
| --- | --- |
| [wit/world.wit](wit/world.wit) | The `extractor` interface plus the `metadata-parser` and `cli-app` worlds. |
| [metadata-parser/app.py](metadata-parser/app.py) | Python metadata extractor, compiled with `componentize-py`. |
| [cli-app/src/lib.rs](cli-app/src/lib.rs) | Rust CLI: async `wasi:cli/run`, file/URL reading, streaming, formatting. |
| [cli-app/Cargo.toml](cli-app/Cargo.toml) | `cdylib` crate using `wasip3` (`http-compat`), `wit-bindgen`, `http`, `http-body-util`. |
| [cli-app/.cargo/config.toml](cli-app/.cargo/config.toml) | Pins the build to `wasm32-wasip2`. |
...
world cli-app         { import extractor; }   // Rust CLI
```
