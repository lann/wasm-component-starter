// End-to-end smoke test for the async-browser-streams example, runnable under
// Node (which, like the browser, drives the transpiled component over JSPI).
//
// It mirrors exactly what the browser does:
//   1. Build `entries` (name + size) and a single ReadableStream of every
...
const entries = files.map((f) => ({ name: f.name, size: BigInt(f.bytes.length) }));

// A ReadableStream that yields each file's bytes in order, in small chunks, to
// exercise the streaming path (never holding everything at once).
